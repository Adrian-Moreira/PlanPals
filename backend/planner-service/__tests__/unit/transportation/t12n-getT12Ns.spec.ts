...
describe('Transportation->getActivitiesById', () => {
  ...
  it('should get existing activities from d9n', async () => {
    t12nMock.expects('findById').resolves(existingTransportation)

    await TransportationService.getTransportationDocumentsByPlannerId(
      req as Request,
      res as Response,
    ...
    expect(req.body.result[0].type).toEqual('test')
    expect(req.body.result[0].details).toEqual('test')
    expect(req.body.result[0].vehicleId).toEqual('test')
    expect(req.body.result[0].departureTime).toEqual(
      existingTransportation.departureTime,
    )
    expect(req.body.result[0].arrivalTime).toEqual(
